...
namespace Hathora.Core.Scripts.Editor.Server
{
    public static class HathoraDocker
    {
        ...
        {
            string shortDateTime = $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}";
            
            return $@"############################################################################
This Dockerfile is auto-generated by {nameof(HathoraDocker)}.cs @ {shortDateTime}
############################################################################

FROM ubuntu

# Copy the server build files into the container
COPY ./{_serverPaths.ExeBuildDir} .
