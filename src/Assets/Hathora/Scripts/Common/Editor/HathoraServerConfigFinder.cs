...
namespace Hathora.Scripts.Common.Editor
{
    ...
    public class HathoraServerConfigFinder : EditorWindow
    {
        ...
        private static List<HathoraClientConfig> serverConfigs;
        ...
        {
            foreach (HathoraClientConfig config in serverConfigs)
            {
        ...
        private void createAndSelectNewConfig()
        {
            ...
            string fullPath = $"{basePath}/HathoraClientConfig.asset";

            // Create Hathora directory if it doesn't exist
            if (!AssetDatabase.IsValidFolder(basePath))
                AssetDatabase.CreateFolder("Assets", "Hathora");

            // Create a new instance of HathoraClientConfig
            HathoraClientConfig newConfig = ScriptableObject.CreateInstance<HathoraClientConfig>();
            ...
            // Select the newly created HathoraClientConfig in the Project tab
        ...
        {
            string[] guids = AssetDatabase.FindAssets("t:Hathora.Scripts.Net.Server.HathoraClientConfig");
            serverConfigs = guids?.Select(guid => AssetDatabase.LoadAssetAtPath<HathoraClientConfig>(
                AssetDatabase.GUIDToAssetPath(guid))).ToList();
        }

        private static void selectHathoraServerConfig(HathoraClientConfig config)
        {
